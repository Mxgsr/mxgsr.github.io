# Historial de cambios y plan pendiente

## Completado

### Fase 1 - HTML basico
- index.html creado con estructura HTML5 y contenido personalizado

### Fase 2 - CSS
- styles.css creado con fuente mono, fondo blanco, layout centrado
- Vinculado a index.html

### Fase 3 - Navegacion y paginas
- blog.html creado con post "Me aburri de consumir"
- portafolio.html creado con proyecto CASEN
- Navegacion entre las 3 paginas

### Fase 4 - GitHub Pages
- Sitio publicado en https://mxgsr.github.io

## En progreso - Fase 5: Tipografia y diseno
- index.html ya tiene: Google Fonts (JetBrains Mono), nav unificado, footer
- Falta: agregar Google Fonts y footer a blog.html y portafolio.html
- Falta: actualizar font-family en styles.css a JetBrains Mono, agregar estilos de footer

## Pendiente

### Fase 6 - Responsive design
- Agregar media queries en styles.css para pantallas chicas (<600px)
- Ajustar: font-size, padding, nav layout

### Fase 7 - Estructura del blog
- Crear carpeta posts/ con un archivo HTML por post
- Mover el post a posts/me-aburri-de-consumir.html
- blog.html pasa a ser un indice con links a cada post

### Fase 8 - Dark mode
- Crear script.js con toggle de tema
- Variables CSS para colores
- Boton de toggle en el nav
- Guardar preferencia en localStorage
